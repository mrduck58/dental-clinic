...
export default function OwnerPageHeader({ title, subtitle, left, right }: OwnerPageHeaderProps) {
    ...
    <header className="sticky top-0 z-20 bg-white/95 backdrop-blur-md border-b border-slate-200 px-4 sm:px-8 h-16 sm:h-20 flex items-center justify-between shrink-0 shadow-sm shadow-slate-100/50">
      <div className="flex items-center gap-3 sm:gap-4 min-w-0">
        ...
        <div className="min-w-0">
          <h1 className="text-lg sm:text-xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2 truncate">{title}</h1>
          {subtitle && <div className="text-[12px] sm:text-[13px] text-slate-400 font-semibold mt-0.5 truncate">{subtitle}</div>}
        </div>
      </div>
