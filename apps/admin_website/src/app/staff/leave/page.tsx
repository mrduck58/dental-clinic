...
const LEAVE_TYPES = ["Phép năm","Nghỉ ốm","Nghỉ việc riêng","Nghỉ không lương","Nghỉ thai sản / Nghỉ phụ sản","Nghỉ bù"];

const MOCK_HISTORY: LeaveRequest[] = [
  { id: "L001", type: "Phép năm",       from: "2026-05-20", to: "2026-05-22", days: 3, reason: "Nghỉ lễ gia đình và sắp xếp việc cá nhân",              status: "approved", submittedAt: "2026-05-15" },
  { id: "L002", type: "Nghỉ ốm",        from: "2026-06-02", to: "2026-06-03", days: 2, reason: "Sốt cao, cảm lạnh, có đơn thuốc của bác sĩ",            status: "approved", submittedAt: "2026-06-01" },
  { id: "L003", type: "Nghỉ việc riêng",from: "2026-06-25", to: "2026-06-25", days: 1, reason: "Giải quyết thủ tục hành chính cá nhân",                 status: "pending",  submittedAt: "2026-06-10" },
  { id: "L004", type: "Phép năm",       from: "2026-04-10", to: "2026-04-11", days: 2, reason: "Du lịch cùng gia đình",                                  status: "rejected", submittedAt: "2026-04-01", note: "Trùng ca trực cuối tuần, vui lòng đổi lịch" },
];
...
export default function StaffLeavePage() {
  ...
  const totalDays = from && to
    ? Math.max(0, Math.ceil((new Date(to).getTime() - new Date(from).getTime()) / 86400000) + 1)
    : 0;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setHistory(prev => [{
      ...
      type, from, to, days: totalDays,
      reason: reason.trim(), status: "pending", submittedAt: "2026-06-13",
    }, ...prev]);
    setSubmitted(true);
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-8 flex-1 overflow-y-auto flex flex-col gap-6">
          ...
          <div className="grid grid-cols-3 gap-5 shrink-0">
            {[
              { label: "Phép năm còn lại", value: "8",               sub: "/ 12 ngày năm 2026",    color: "text-primary",     bg: "bg-red-50"    },
