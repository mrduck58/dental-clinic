...
export default function StaffLeavePage() {
  ...
  // Ca đã nằm trong một đơn Pending/Approved khác — khoá lại để không xin nghỉ trùng lần nữa.
  // Đơn bị từ chối/huỷ không tính, ca đó lại được chọn bình thường.
  const lockedShifts = useMemo(() => {
    const set = new Set<string>();
    for (const req of history) {
      if (req.status !== "Pending" && req.status !== "Approved") continue;
      for (const sh of req.shifts) set.add(selectionKey(sh.date, sh.shiftId));
    }
    return set;
  }, [history]);
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <Portal>
          <div className="fixed top-24 right-8 z-50 w-[min(28rem,calc(100vw-4rem))] space-y-3 pointer-events-none">
            {submitted && (
              <div className="animate-fade-in pointer-events-auto flex items-center gap-3 bg-green-50 border border-green-200 text-green-700 px-5 py-3 rounded-2xl text-[13.5px] font-bold shadow-lg shadow-green-900/5">
                ...
                <span className="flex-1">Đơn đã gửi thành công. Chờ quản lý duyệt.</span>
              </div>
            )}
            {error && (
              <div className="animate-fade-in pointer-events-auto flex items-center gap-3 bg-rose-50 border border-rose-200 text-rose-700 px-5 py-3 rounded-2xl text-[13.5px] font-bold shadow-lg shadow-rose-900/5">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-4 sm:p-6 flex flex-col gap-5">
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <StaffLeaveShiftPicker selected={selected} onToggle={toggleShift} onToggleDay={toggleDay} lockedShifts={lockedShifts} />
              </div>
