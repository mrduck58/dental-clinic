...
const INIT_PLANS: TreatmentPlan[] = [
  {
    id: "TP001", patientName: "Nguyễn Văn An", patientPhone: "0901 234 567", gender: "Nam",
    dentist: "BS. Thảo", date: "13/06/2026", diagnosis: "Răng khôn mọc ngầm hàm dưới bên trái",
    procedures: [
      { name: "Chụp X-quang panoramic",         qty: 1, price: 350000  },
      { name: "Nhổ răng khôn có gây mê",         qty: 1, price: 1500000 },
      { name: "Kháng sinh & giảm đau (7 ngày)",  qty: 1, price: 250000  },
    ],
  },
  {
    id: "TP002", patientName: "Trần Thị Bích", patientPhone: "0912 345 678", gender: "Nữ",
    dentist: "BS. Minh", date: "13/06/2026", diagnosis: "Sâu răng số 6 độ 3 hàm dưới",
    procedures: [
      { name: "Kiểm tra & chụp X-quang",          qty: 1, price: 200000  },
      { name: "Điều trị tủy răng số 6",            qty: 1, price: 2500000 },
      { name: "Trám composite màu răng",            qty: 1, price: 800000  },
    ],
  },
  {
    id: "TP003", patientName: "Hoàng Văn Đức", patientPhone: "0945 678 901", gender: "Nam",
    dentist: "BS. Minh", date: "13/06/2026", diagnosis: "Mất răng số 4, 5 hàm trên bên phải",
    procedures: [
      { name: "Chụp CT cone beam 3D",              qty: 1, price: 1200000  },
      { name: "Cấy ghép Implant (trụ titanium)",   qty: 2, price: 8500000  },
      { name: "Phẫu thuật ghép xương tự thân",     qty: 1, price: 3500000  },
      { name: "Bọc mão sứ Zirconia toàn phần",     qty: 2, price: 4500000  },
    ],
  },
];

const INIT_PENDING: Invoice[] = [
  {
    id: "INV003", patientName: "Phạm Minh Cường", patientPhone: "0923 456 789", gender: "Nam",
    dentist: "BS. Thảo", date: "13/06/2026",
    items: [
      { name: "Kiểm tra định kỳ",      qty: 1, price: 300000 },
      { name: "Lấy cao răng siêu âm",  qty: 1, price: 500000 },
    ],
    subtotal: 800000, discount: 0, finalTotal: 800000,
    paymentMethod: "app", status: "app_waiting", note: "",
  },
  {
    id: "INV004", patientName: "Lê Thu Hà", patientPhone: "0934 567 890", gender: "Nữ",
    dentist: "BS. Linh", date: "13/06/2026",
    items: [
      { name: "Tẩy trắng răng Zoom Advanced", qty: 1, price: 3500000 },
    ],
    subtotal: 3500000, discount: 350000, finalTotal: 3150000,
    paymentMethod: null, status: "pending", note: "Khách hàng thân thiết 10%",
  },
];

const INIT_PAID: Invoice[] = [
  {
    id: "INV001", patientName: "Vũ Thị Ngọc", patientPhone: "0956 789 012", gender: "Nữ",
    dentist: "BS. Linh", date: "12/06/2026",
    items: [{ name: "Bọc răng sứ Emax", qty: 3, price: 5800000 }],
    subtotal: 17400000, discount: 0, finalTotal: 17400000,
    paymentMethod: "transfer", status: "paid", note: "",
  },
  {
    id: "INV002", patientName: "Đỗ Quang Huy", patientPhone: "0967 890 123", gender: "Nam",
    dentist: "BS. Thảo", date: "12/06/2026",
    items: [{ name: "Lấy cao răng siêu âm", qty: 1, price: 500000 }],
    subtotal: 500000, discount: 0, finalTotal: 500000,
    paymentMethod: "cash", status: "paid", note: "",
  },
  {
    id: "INV000", patientName: "Mai Thị Thanh", patientPhone: "0943 567 890", gender: "Nữ",
    dentist: "BS. Minh", date: "11/06/2026",
    items: [{ name: "Niềng răng Invisalign — đặt cọc đợt 1", qty: 1, price: 12000000 }],
    subtotal: 12000000, discount: 1200000, finalTotal: 10800000,
    paymentMethod: "app", status: "paid", note: "Đặt cọc lần 1",
  },
];
...
let invCounter = 5;
const nextInvId = () => `INV${String(invCounter++).padStart(3, "0")}`;
...
function PlansTab({ plans, onIssued }: {
  ...
  const selectPlan = (p: TreatmentPlan) => {
    ...
    setDiscount(0); setNote(""); setMethod(null); setSaved(false);
  };
              ...
              <button key={p.id} onClick={() => selectPlan(p)}
                ...
                <div className="flex flex-col gap-2.5 px-4 py-4 flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[11.5px] font-semibold text-slate-400">{p.procedures.length} dịch vụ</span>
                    <span className="text-[13px] font-black text-slate-700">{fmt(sum(p.procedures))}</span>
                  </div>
                </div>
              </button>
            );
          ...
          <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
            ...
            <div className="px-7 py-6 flex flex-col gap-6">
              ...
              <div className="flex items-center gap-4 pt-1 border-t border-slate-100">
                <button onClick={handleIssue} disabled={!method || items.length === 0}
...
function HistoryTab({ paid }: { paid: Invoice[] }) {
  const todayRevenue = paid
    .filter(i => i.date === "13/06/2026")
    .reduce((s, i) => s + i.finalTotal, 0);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Doanh thu hôm nay",   value: fmt(todayRevenue), icon: "M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z", color: "text-emerald-600", bg: "bg-emerald-50" },
          ...
          { label: "Trung bình / hóa đơn", value: paid.length > 0 ? fmt(Math.round(paid.reduce((s, i) => s + i.finalTotal, 0) / paid.length)) : "—",                                                                                                                                                                                                                                                                                                                                                                                                                                                      icon: "M7.5 14.25v2.25m3-4.5v4.5m3-6.75v6.75m3-9v9M6 20.25h12A2.25 2.25 0 0020.25 18V6A2.25 2.25 0 0018 3.75H6A2.25 2.25 0 003.75 6v12A2.25 2.25 0 006 20.25z", color: "text-violet-600", bg: "bg-violet-50"  },
        ].map(s => (
              ...
              return (
                <tr key={inv.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <div className="font-black text-slate-900 font-mono text-[13.5px]">{fmt(inv.finalTotal)}</div>
...
export default function InvoicesPage() {
  ...
  const [tab,     setTab]     = useState<"plans" | "pending" | "history">("plans");
  const [plans,   setPlans]   = useState<TreatmentPlan[]>(INIT_PLANS);
  const [pending, setPending] = useState<Invoice[]>(INIT_PENDING);
  const [paid,    setPaid]    = useState<Invoice[]>(INIT_PAID);

  const handleIssued = (inv: Invoice) => {
    setPlans(prev => prev.filter(p => p.id !== inv.planId));
    setPending(prev => [...prev, inv]);
  ...
  const handlePaid = (id: string) => {
    ...
    setPaid(prev => [...prev, { ...inv, status: "paid" }]);
  };
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-8 flex-1 overflow-y-auto flex flex-col gap-5">
          ...
          <div className="flex gap-2">
            {([
              { key: "plans",   label: "Liệu trình → Hóa đơn", count: plans.length,   dot: plans.length > 0 },
              { key: "pending", label: "Chờ thanh toán",         count: pending.length, dot: pending.some(i => i.status === "app_waiting") },
              { key: "history", label: "Lịch sử hóa đơn",        count: paid.length,    dot: false },
            ] as const).map(t => (
          ...
          {tab === "plans"   && <PlansTab   plans={plans}      onIssued={handleIssued} />}
          {tab === "pending" && <PendingTab invoices={pending}  onPaid={handlePaid}    />}
          {tab === "history" && <HistoryTab paid={paid}                                />}
        </div>
      </main>
    </div>
  );
