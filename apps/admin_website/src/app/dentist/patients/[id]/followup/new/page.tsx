...
const TIME_SLOTS = ["07:30","08:00","08:30","09:00","09:30","10:00","10:30","11:00","13:00","13:30","14:00","14:30","15:00","15:30","16:00","16:30"];
...
export default function NewFollowUpPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    setTimeout(() => router.push(`/dentist/patients/${id}`), 800);
  };
                  ...
                  <button key={t} type="button" onClick={() => setTimeSlot(t)}
                    className={`py-2.5 text-[13px] font-bold rounded-xl border transition-all cursor-pointer ${
                      timeSlot === t
                        ...
                        : "border-slate-200 text-slate-600 hover:border-primary/40 hover:bg-red-50 hover:text-primary"
                    }`}>
                    {t}
                  </button>
                ))}
              </div>
              {!timeSlot && <p className="text-[12px] text-slate-400 font-semibold">Chưa chọn giờ</p>}
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="submit" disabled={saving || !timeSlot}
                ...
                {saving ? (
                  ...
                  <><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg>Xác nhận lịch hẹn</>
                )}
              </button>
            </div>
          </form>
