...
import { createTreatmentPlanApi, addTreatmentStepApi, getServicesApi, type ServiceDto } from "../../../../../../lib/apiClient";
...
function NewTreatmentPageContent() {
  ...
  const handleSave = async () => {
    ...
    try {
      const plan = await createTreatmentPlanApi(id, {
        description: `Liệu trình điều trị - ${items.length} bước`,
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        await addTreatmentStepApi({
          ...
          stepNumber: i + 1,
          description: `${item.serviceName} - Răng ${item.teeth.join(", ")}`,
              ...
              <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
                ...
                <div className="p-5 flex flex-col gap-4">
                  ...
                  >
                    ...
                    Thêm bước điều trị{canAdd ? ` (${sel.size} răng)` : ""}
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden flex flex-col h-fit sticky top-8">
              ...
              {items.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center gap-3 py-12 text-center px-4">
                  ...
                  <p className="text-[13px] font-semibold text-slate-400">Chưa có bước nào</p>
                  <p className="text-[11px] text-slate-400">Chọn răng và dịch vụ để thêm bước</p>
                </div>
              ) : (
            ...
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
              <div className="px-6 py-4 bg-gradient-to-r from-primary/5 to-red-50/50 border-b border-slate-100 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-[13px] text-slate-500">{items.length} bước điều trị</span>
