...
import { createMedicineApi, uploadFileApi } from "../../../../lib/apiClient";
...
export default function AddMedicinePage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        const uploaded = await uploadFileApi(imageFile);
      ...
      setToast({ show: true, message: "Thêm thuốc thành công!" });
      setTimeout(() => {
        window.location.href = "/dashboard/medicines";
      }, 2000);
    } catch (err) {
      setSaveError(err instanceof Error ? err.message : "Thêm thuốc thất bại.");
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-20 bg-white/95 backdrop-blur-md border-b border-slate-200 px-8 h-20 flex items-center justify-between shrink-0 font-sans shadow-sm shadow-slate-100/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Thêm thuốc mới</h1>
