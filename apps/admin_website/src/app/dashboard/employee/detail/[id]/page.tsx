            ...
            <button
              onClick={() => {
                ...
                router.push(isDoctor
                  ? `/dashboard/employee/edit-doctor/${staff.id}`
                  : `/dashboard/employee/edit-staff/${staff.id}`
                );
              }}
              ...
              <div className="md:col-span-2 flex flex-col gap-5">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                      ...
                      {calDays.map(({ date, inMonth }) => {
                        ...
                        return (
                          <div key={date} className={`
                            ...
                            ${!inMonth ? "opacity-20 border-transparent" : isToday ? "border-primary shadow-sm shadow-primary/15" : "border-slate-100"}
                          `}>
                            <div className={`text-center text-[10px] font-black py-1 leading-none ${isToday ? "text-primary bg-primary/5" : "text-slate-600"}`}>
                              {dayNum}
                            </div>
                            <div className={`h-1.5 ${hasMorning ? "bg-sky-400" : inMonth ? "bg-slate-100" : "bg-transparent"}`} />
                            <div className={`h-1.5 ${hasAfternoon ? "bg-amber-400" : inMonth ? "bg-slate-100" : "bg-transparent"}`} />
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div className="flex items-center gap-3 mt-3 pt-3 border-t border-slate-50 flex-wrap">
                    <div className="flex items-center gap-1.5">
                      <div className="w-4 h-1.5 rounded-sm bg-sky-400" />
                      <span className="text-[10.5px] font-bold text-slate-400">Ca sáng</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="w-4 h-1.5 rounded-sm bg-amber-400" />
                      <span className="text-[10.5px] font-bold text-slate-400">Ca chiều</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="w-4 h-1.5 rounded-sm bg-slate-100 border border-slate-200" />
                      <span className="text-[10.5px] font-bold text-slate-400">Nghỉ</span>
                    </div>
                    <div className="flex items-center gap-1.5 ml-auto">
                      <div className="w-3.5 h-3.5 rounded border-2 border-primary" />
