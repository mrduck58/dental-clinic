...
export default function OwnerPayrollDentistsPage() {
  useRequireOwner();
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const data = await getPayrollPeriodApi({
        ...
        // Trang này chỉ dành riêng cho nha sĩ — lọc theo vai trò
        role: "Dentist,Doctor",
      });
      setPeriod(data);
      setErrorMsg(null);
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "NhaSi");
    XLSX.writeFile(workbook, `Bang_Luong_Nha_Si_Thang_${selectedMonth}_Nam_${selectedYear}.xlsx`);
  };
  ...
  return (
    <div className="animate-fade-in flex min-h-screen bg-slate-50 font-sans text-slate-800">
      <OwnerSidebar activeMenu="payroll-dentists" />

      <main className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <OwnerPageHeader
          title="Bảng Lương Nha Sĩ"
          subtitle="Quản lý đãi ngộ và chi trả lương hàng tháng cho nha sĩ."
        />
        ...
        <div className="flex-1 p-8 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm">
              ...
              <span className="text-[11px] text-slate-400 font-semibold block mt-0.5">
                Của {totalStaff} nha sĩ {isFiltered ? "khớp bộ lọc hiện tại" : "của phòng khám"}
              </span>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col gap-3.5">
            ...
            {/* Page Size Selector — hàng dưới, tách riêng khỏi bộ lọc tháng/năm */}
