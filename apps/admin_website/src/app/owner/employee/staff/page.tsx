...
type SortKey = "name" | "department" | "status" | "salary" | "leaveAccrued";
...
export default function OwnerStaffManagementPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([stLeave, stActive, pendingLeaves]) => {
      setOnLeaveCount(stLeave.totalCount);
      setBaseTotal(stActive.totalCount + stLeave.totalCount);
  ...
  const offTodayStaff     = Math.max(0, baseTotal - workingTodayStaff);
  ...
  const statCards = [
    { label: "Tổng nhân viên",   sub: "Lễ tân và trợ lý",         value: baseTotal,          numClass: "text-slate-900",  bg: "bg-slate-50",  iconCls: "text-slate-500",  icon: ICON_USERS  },
  ...
  const handleExportExcel = () => {
    const headers = ["Họ tên", "Email", "SĐT", "Vai trò", "Bộ phận", "Trạng thái"];
    ...
    <div className="animate-fade-in flex min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <OwnerPageHeader
          ...
          subtitle="Tra cứu hồ sơ nhân viên phục vụ vận hành phòng khám."
        />
        ...
        <div className="p-8 flex-1 overflow-y-auto flex flex-col gap-6">
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col gap-4 shrink-0">
            <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3.5 flex-wrap">
              ...
              {/* Add */}
              <button
                onClick={() => router.push("/owner/employee/add-staff")}
              ...
              <table className="w-full text-[13.5px] text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-150 bg-slate-50/70 select-none">
                    ...
                    <SortableTh column="department" label="Chức vụ / Bộ phận" sortKey={sortKey} sortDir={sortDir} onSort={handleSort} className="px-6" />
                  ...
                  {isLoading ? (
                    <tr>
                      <td colSpan={8} className="px-6 py-16 text-center font-bold text-slate-400 animate-pulse">
                        Đang tải danh sách hồ sơ nhân viên...
                      </td>
                    </tr>
                  ) : staffList.length > 0 ? (
                    staffList.map((item) => {
                        ...
                        >
                          ...
                          <td className="px-6 py-4.5">
                            {(() => {
                              ...
                              const type = item.employmentType || (isShift ? "Shift-based" : "Full-time");
                              return (
                                <div className="group relative inline-flex items-center gap-1">
                                  ...
                                  <span className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-1 px-2 py-1 bg-slate-900 text-white text-[10px] rounded opacity-0 group-hover:opacity-100 transition-opacity duration-150 pointer-events-none whitespace-nowrap z-30 shadow-md font-extrabold">
                                    {type === "Full-time" ? "Toàn thời gian (Full-time)" : "Theo ca (Shift-based)"}
                                  </span>
                                </div>
                              );
                            })()}
                            ...
                            <div className="flex items-center justify-center gap-1">
                              <button
                                onClick={() => {
                                  ...
                                  router.push(`/owner/employee/edit-staff/${item.id}`);
                                }}
                                title="Chỉnh sửa"
