...
export default function EditDentistPage() {
  ...
  const populateData = (data: StaffDto) => {
    ...
    setFormData({
      ...
      servicesHandled: data.servicesHandled || "Nhổ răng, Lấy cao răng",
      startDate: fmtDateInput(data.startDate),
      certificateIssuedDate: fmtDateInput(data.certificateIssuedDate),
  ...
  useEffect(() => {
    ...
    if (id) {
      getStaffByIdApi(id)
        ...
        .catch((err) => {
          setErrors({ api: err instanceof Error ? err.message : "Không thể tải thông tin nha sĩ" });
        });
    }
  }, [id]);
    ...
    return (
      <div className="animate-fade-in flex min-h-screen bg-slate-50 font-sans text-slate-800">
        ...
        <main className="flex-1 flex flex-col items-center justify-center gap-4">
          ...
          <button onClick={() => router.push("/owner/employee/dentists")} className="px-6 py-2.5 bg-primary hover:bg-primary-hover text-white font-extrabold rounded-xl transition-all cursor-pointer shadow-md">
            Quay lại danh sách
          </button>
        </main>
      </div>
  ...
  return (
    <div className="animate-fade-in flex min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <OwnerPageHeader
          left={
            <button onClick={() => router.push("/owner/employee/dentists")} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-xl transition-all cursor-pointer">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 w-full">
              ...
              <div className="flex items-center justify-between border-b border-slate-200 pb-4 mb-2 shrink-0">
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => router.push("/owner/employee/dentists")}
