...
export default function OwnerFeedbackDashboardPage() {
  useRequireOwner();
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-20 bg-white/95 backdrop-blur-md border-b border-slate-200 px-8 h-20 flex items-center justify-between shrink-0 font-sans shadow-sm shadow-slate-100/50">
          <div>
            ...
            <p className="text-[13px] text-slate-400 font-semibold mt-0.5">Quản lý và phản hồi ý kiến đóng góp từ khách hàng</p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden flex flex-col">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-[14px]">
                <thead>
                  <tr className="bg-slate-50/50 font-extrabold text-slate-400 uppercase tracking-wider border-b border-slate-150 border-collapse">
              ...
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                <div className="flex items-center gap-0.5">
                  {Array.from({ length: 5 }).map((_, idx) => (
                    <svg
                      key={idx}
                      className={`w-3.5 h-3.5 ${idx < replyTarget.rating ? "text-amber-400 fill-amber-400" : "text-slate-250"}`}
                      viewBox="0 0 24 24"
                      fill="currentColor"
                    >
                      <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" />
