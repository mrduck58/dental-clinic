...
public record UpdateExpenseCommand(Guid Id, UpdateExpenseRequest Request) : IRequest<ExpenseDto>;

public class UpdateExpenseHandler(
    IExpenseRepository expenseRepository,
    IActivityLogService activityLogService,
    ICurrentUserService currentUser) : IRequestHandler<UpdateExpenseCommand, ExpenseDto>
{
    public async Task<ExpenseDto> Handle(UpdateExpenseCommand command, CancellationToken ct)
    {
        ...
        expense.Update(category, r.Description, r.Amount, r.Date, r.Note, r.IsRecurring, frequency);
        ...
        await activityLogService.LogAsync(
            ...
            description: $"Sửa chi phí: {expense.Description} ({expense.Amount:#,##0}đ)",
