...
public interface IDentistReviewRepository
{
    ...
    Task<DentistReview?> GetByAppointmentIdAsync(Guid appointmentId, CancellationToken ct = default);
    Task<int> CountByDentistAndPatientAsync(Guid dentistId, Guid patientId, CancellationToken ct = default);
